...
mAGN & Radio$\to$gamma chain & 2.37 & Di Mauro+2014 \\
SFG & IR$\to$gamma chain & 2.70 & Gruppioni+2013 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Smooth inverse-sum evolution} (Ajello+2012 Eq.~15, D12):
\begin{equation}
...
\textbf{4-step chain} (Di Mauro+2014 / Pinetti 2022 Eq.~C.19):
\begin{enumerate}
    \item $L_\gamma \to \nu L_{\nu,\mathrm{core}}^{5\mathrm{GHz}}$: \quad $\log L_\gamma = 2.0 + 1.008\log(\nu L_\nu)$ [erg/s]
    \item $\nu L_\nu \to L_{\mathrm{core}}^{5\mathrm{GHz}}$: \quad divide by $\nu\cdot 10^7$ [W/Hz]
    \item $L_{\mathrm{core}} \to L_{\mathrm{tot}}^{1.4\mathrm{GHz}}$: \quad Lara (2004): $\log L_{\mathrm{core}} = 4.2 + 0.77\log L_{\mathrm{tot}}$
    \item $L_{\mathrm{tot}}^{1.4} \to L_{151}$: \quad $(1400/151)^{-0.80}$ [Inoue 2011]
\end{enumerate}
...
\textbf{Gruppioni+2013 modified Schechter (3 components):}
\begin{equation}
...
\vspace{0.3cm}
\textbf{Ackermann+2012 $L_\gamma$--$L_{\mathrm{IR}}$ scaling:}
\begin{equation}
...
\begin{itemize}
    \item \texttt{'pinetti\_constant'}: $\Fsens = 10^{-10}\cmtwo$ (Pinetti+2020)
    \item \texttt{'4fgl\_dr4\_psf'}: $\Fsens = 7.3{\times}10^{-11}\cmtwo$ (Ballet+2023) $+$ PSF scaling
\end{itemize}
\end{frame}
