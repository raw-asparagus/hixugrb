...
|-------|---------------------|----------|------------------------|--------|
| $E(z) = \sqrt{\Omega_M(1+z)^3 + \Omega_\Lambda}$ | Standard flat LCDM | `cosmology.py:E(z)` lines 161–164 | Same | **Match** |
| $H(z) = H_0 E(z)$; $H_0 = 67.36$ km/s/Mpc | Planck 2018 | `cosmology.py:H(z)` lines 167–169 | Same | **Match** |
| $\chi(z) = (c/H_0)\int_0^z dz'/E(z')$ | Standard cosmology | `cosmology.py:chi(z)` lines 181–202, returns Mpc/h; uses precomputed `_chi_interp` spline (built at init, lines 123–128) with `_chi_scalar` fallback (lines 172–178) | Same | **Match** |
| $P_{\rm lin}(k,z)$ via CAMB Boltzmann solver | Planck 2018 params | `cosmology.py:P_lin(k,z)` lines 251–270 | Same | **Match** |
| $\sigma^2(R,z) = \frac{1}{2\pi^2}\int dk\,k^2 P_{\rm lin} W^2(kR)$ | Perturbation theory | `cosmology.py:sigma_R(R,z)` lines 287–303 | Same | **Match** |
| $W(x) = 3(\sin x - x\cos x)/x^3$ (top-hat) | Standard | `cosmology.py:_tophat_W(kR)` lines 277–284 | Same | **Match** |
...
|-------|---------------------|----------|------------------------|--------|-------|
| $R_{\rm vir} = (3M/4\pi\Delta_{\rm vir}(z)\rho_c(z))^{1/3}$ | Thesis Eqs. 3.26–3.28: z-dependent $\Delta_{\rm vir}$ from Bryan & Norman | `halo_model.py:R_vir(M,z)` lines 43–56 with Bryan & Norman $\Delta_{\rm vir}(z)$ lines 24–41 | Same | **Match** | Repository now matches the thesis choice for the virial definition |
| $v_c = \sqrt{GM/R_{\rm vir}}$ | Thesis Eq. 4.4 | `halo_model.py:v_circ(M,z)` lines 73–87 | Same | **Match** | Unit handling: M_phys=M/h, R_phys=Rv/h, verified correct |
...
| $b(\nu) = 1 + (q\nu-1)/\delta_c + 2p/[\delta_c(1+(q\nu)^p)]$ | Sheth & Tormen (1999); thesis Eq. 3.51 | `halo_model.py:bias(M,z)` lines 111–124 | Same | **Match** | Inherits $q$ difference from mass function |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $M_{\rm HI} = \alpha f_{H,c} M (M/10^{11}h^{-1}M_\odot)^\beta \exp[-(v_{c,0}/v_c)^3]$ | Padmanabhan+ (2017) Eq. 1; thesis Eq. 4.2 | `hi_model.py:M_HI(M,z)` lines 33–48 | Same | **Match** | |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $c_{\rm HI} = c_{HI,0}(M/10^{11}M_\odot)^{-0.109} \times 4/(1+z)^\gamma$ | Padmanabhan+ Eq. 3; thesis Eq. 4.10 | `hi_model.py:c_HI(M,z)` lines 50–61 with `M / cfg.h / 1e11` | Same | **Match** | The current implementation converts the repository mass variable from $M_\odot/h$ to $M_\odot$ before applying the Padmanabhan pivot |
...
| Exponent $-0.109$ | Padmanabhan+ Eq. 3 | Hardcoded in `hi_model.py:61` | Same | **Match** | |

### 3.3 HI density profile

| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $\rho_{\rm HI}(r) = \rho_0 r_s^3 / [(r+0.75r_s)(r+r_s)^2]$ | Padmanabhan+ Eq. A1; thesis Eq. 4.8 | `hi_model.py:u_HI` integrand, lines 101–132 | Same | **Match** | Modified NFW from Maller & Bullock (2004) |
| $\rho_0$ from $\int_0^{R_{\rm vir}} 4\pi r^2 \rho_{\rm HI}\,dr = M_{\rm HI}$ | Padmanabhan+ Eq. A2; thesis Eq. 4.9 | `hi_model.py:rho0_HI(M,z)` lines 86–99 | Same | **Match** | |
| Normalization integral: partial fractions A=9, B=−8, C=−4 | Analytic result | `hi_model.py:_hi_profile_norm_integral` lines 68–84 | Same | **Match** | Verified: $x^2/[(x+0.75)(x+1)^2]$ decomposes as $9/(x+0.75) - 8/(x+1) - 4/(x+1)^2$. Coefficient checks: $x^2$: $A+B = 9-8 = 1$ ✓; $x^1$: $2A+1.75B+C = 18-14-4 = 0$ ✓; $x^0$: $A+0.75B+0.75C = 9-6-3 = 0$ ✓ |

### 3.4 Fourier transform of HI profile

| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $\tilde{u}_{\rm HI} = (4\pi/M_{\rm HI})\int_0^{R_{\rm vir}} r^2 \rho_{\rm HI} \sin(kr)/(kr)\,dr$ | Thesis Eq. 4.11 | `hi_model.py:u_HI(k,M,z)` lines 101–132 | Same | **Match** | No analytic FT exists for modified NFW; numerical quadrature is the correct approach |
| $\tilde{u}_{\rm HI}(k\to0) = 1$ normalization | Standard convention | `u_HI` returns 1.0 for $k \lt 10^{-10}$ | Same | **Match** | |
| Numerical integration: `scipy.quad`, `epsrel=1e-6`, `limit=200` | N/A (computational choice) | `hi_model.py:128` | Same | Appropriate | Expensive but accurate |

### 3.5 Mean HI density

| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $\bar\rho_{\rm HI}(z) = \int (dn/dM)\,M_{\rm HI}\,dM$ | Thesis Eq. 4.5 | `hi_model.py:rho_HI_mean(z)` lines 178–190 (dispatcher); integration in `_rho_HI_default` lines 145–149 and `_rho_HI_scalar` lines 153–157 | Same | **Match** | Integration via `scipy.quad` in log-mass; cached via `@_cache_stable` |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $\Omega_{\rm HI}(z) = \bar\rho_{\rm HI}/\rho_{c,0}$ | Thesis Eq. 4.6 | `hi_model.py:Omega_HI(z)` lines 192–204 | Fixed $2.45\times10^{-4}$ | **Differs** | Thesis (p.122) subsequently uses fixed $\Omega_{\rm HI} = 2.45\times10^{-4}$. Pipeline computes from the halo integral (z-dependent); no $(1+z)^{-3}$ factor because `rho_HI_mean` already returns the comoving density. Pipeline is more physical; thesis acknowledges up to factor-2 variation. Propagates linearly into $\bar{T}_b$ and $W_{\rm HI}$. A third mode is also available: `Omega_HI_cunnington(z)` (lines 234–244) returns the Cunnington et al. (2025) Eq. A5 polynomial, used when `hi_brightness='cunnington'` |

### 3.7 Brightness temperature

| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $\bar{T}_b = 188\,h\,\Omega_{\rm HI}(1+z)^2/E(z)$ mK | Thesis Eq. 3.4 uses $44\,\mu{\rm K}\times(\Omega_{\rm HI}h/2.45\times10^{-4})(1+z)^2/E(z)$ | `hi_model.py:T_bar_b(z)` lines 206–215; dispatched via `T_bar_b_for_model(z, hi_brightness)` lines 274–293. Cunnington mode uses `T_bar_b_cunnington(z)` lines 260–272 with 180 mK prefactor | 44 $\mu$K via `pinetti2022.T_bar_b_thesis()` | **Partial** | Equivalent: $188 \times 2.45\times10^{-4} \approx 0.046$ mK $= 46\,\mu$K $\approx 44\,\mu$K (rounding). Combined with computed vs fixed $\Omega_{\rm HI}$, numerical results differ slightly. A fixed-Omega mode (`T_bar_b_fixed_omega`, lines 217–232) and the Cunnington 180 mK mode are also available via the `hi_brightness` dispatcher |

### 3.8 Effective HI bias

| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $b_{\rm HI} = (1/\bar\rho_{\rm HI})\int (dn/dM)\,M_{\rm HI}\,b(M)\,dM$ | Thesis Eq. 4.7 | `hi_model.py:b_HI(z, hi_brightness)` lines 295–325; halo integral via `_b_HI_default(z)` lines 161–176 (cached via `@_cache_stable`). When `hi_brightness='cunnington'`, returns polynomial `b_HI_cunnington(z)` lines 246–258 instead | $q=0.75$ via `pinetti2022.b_HI_pinetti()` | **Match** | Via `scipy.quad` in log-mass (halo-integral mode); or Cunnington+2025 Eq. A3 polynomial (cunnington mode) |

### 3.9 HI power spectra

| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $P_{\rm HI}^{\rm 1h} = (1/\bar\rho_{\rm HI}^2)\int (dn/dM)\,M_{\rm HI}^2\,\tilde{u}_{\rm HI}^2\,dM$ | Thesis Eq. 4.13 | `hi_model.py:P_HI_1h` lines 327–348 | Same | **Match** | Rectangle rule over log-mass grid (n_M=160) |
| $P_{\rm HI}^{\rm 2h} = [(1/\bar\rho_{\rm HI})\int (dn/dM)\,b\,M_{\rm HI}\,\tilde{u}_{\rm HI}\,dM]^2 P_{\rm lin}$ | Thesis Eq. 4.14 | `hi_model.py:P_HI_2h` lines 386–406; when `hi_brightness='cunnington'`, collapses to $b_{\rm HI,cunn}^2 P_{\rm lin}$ (lines 398–400), dropping the halo-model integral | Same | **Match** | Rectangle rule over log-mass grid (n_M=160) in halo-integral mode; linear-bias shortcut in cunnington mode |

### 3.10 Window function

| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $W_{\rm HI}(\chi) = \bar{T}_b\,\phi(z)\,H(z)/(c\cdot h)$ | Thesis Eqs. 5.11–5.12 for the per-$z$ window; repository keeps $b_{\rm HI}$ in $P_{\rm HI}$ rather than in `W_HI()` | `hi_model.py:W_HI(z,z_min,z_max,hi_brightness)` lines 408–426; dispatches $\bar{T}_b$ via `T_bar_b_for_model` (line 424) | `pinetti2022.W_HI_pinetti()` | **Match** | $\phi(z) = 1/(z_{\max}-z_{\min})$ top-hat; $H/(c\cdot h)$ Jacobian converts per-$z$ to per-Mpc/$h$ |
| Returns 0 outside $[z_{\min}, z_{\max}]$ | Top-hat selection | `hi_model.py:423` | Same | **Match** | |
...
| $k = (\ell+1/2)/\chi$ | LoVerde & Afshordi (2008); thesis uses $k = \ell/\chi$ | `angular_power.py:175` | $k = \ell/\chi$ | **Differs** | Pipeline improvement: more accurate at low $\ell$. ~5% at $\ell=10$, rapidly decreasing |
...
| Redshift integration: rectangle rule, $n_z=200$ uniform points | N/A (computational choice) | `angular_power.py:154–155` | Same | Appropriate | Sub-percent accuracy for typical band widths |
...
| ~~D3~~ | ~~`from functools import lru_cache` imported but unused~~ | — | — | **Resolved:** dead import removed; replaced by `from .cache import _cache_stable` (used on `_rho_HI_default`, `_rho_HI_scalar`, `_b_HI_default`) |
