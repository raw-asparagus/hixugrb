...
|-------|---------------------|----------|------------------------|--------|
| $E(z) = \sqrt{\Omega_M(1+z)^3 + \Omega_\Lambda}$ | Standard flat LCDM | `cosmology.py:E(z)` | Same | **Match** |
| $H(z) = H_0 E(z)$; $H_0 = 67.36$ km/s/Mpc | Planck 2018 | `cosmology.py:H(z)` | Same | **Match** |
| $\chi(z) = (c/H_0)\int_0^z dz'/E(z')$ | Standard cosmology | `cosmology.py:chi(z)`, returns Mpc/h | Same | **Match** |
| $P_{\rm lin}(k,z)$ via CAMB Boltzmann solver | Planck 2018 params | `cosmology.py:P_lin(k,z)` | Same | **Match** |
| $\sigma^2(R,z) = \frac{1}{2\pi^2}\int dk\,k^2 P_{\rm lin} W^2(kR)$ | Perturbation theory | `cosmology.py:sigma_R(R,z)` | Same | **Match** |
| $W(x) = 3(\sin x - x\cos x)/x^3$ (top-hat) | Standard | `cosmology.py:_tophat_W(kR)` | Same | **Match** |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $M_{\rm HI} = \alpha f_{H,c} M (M/10^{11}h^{-1}M_\odot)^\beta \exp[-(v_{c,0}/v_c)^3]$ | Padmanabhan+ (2017) Eq. 1; thesis Eq. 4.2 | `hi_model.py:M_HI(M,z)` line 30 | Same | **Match** | |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $\rho_{\rm HI}(r) = \rho_0 r_s^3 / [(r+0.75r_s)(r+r_s)^2]$ | Padmanabhan+ Eq. A1; thesis Eq. 4.8 | `hi_model.py:u_HI` integrand, line 126 | Same | **Match** | Modified NFW from Maller & Bullock (2004) |
| $\rho_0$ from $\int_0^{R_{\rm vir}} 4\pi r^2 \rho_{\rm HI}\,dr = M_{\rm HI}$ | Padmanabhan+ Eq. A2; thesis Eq. 4.9 | `hi_model.py:rho0_HI(M,z)` | Same | **Match** | |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $\bar\rho_{\rm HI}(z) = \int (dn/dM)\,M_{\rm HI}\,dM$ | Thesis Eq. 4.5 | `hi_model.py:rho_HI_mean(z)` | Same | **Match** | Integration via `scipy.quad` in log-mass |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $\Omega_{\rm HI}(z) = (1+z)^{-3}\bar\rho_{\rm HI}/\rho_c$ | Thesis Eq. 4.6 | `hi_model.py:Omega_HI(z)` | Fixed $2.45\times10^{-4}$ | **Differs** | Thesis (p.122) subsequently uses fixed $\Omega_{\rm HI} = 2.45\times10^{-4}$. Pipeline computes from the halo integral (z-dependent). Pipeline is more physical; thesis acknowledges up to factor-2 variation. Propagates linearly into $\bar{T}_b$ and $W_{\rm HI}$ |

### 3.7 Brightness temperature

| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $\bar{T}_b = 188\,h\,\Omega_{\rm HI}(1+z)^2/E(z)$ mK | Thesis Eq. 3.4 uses $44\,\mu{\rm K}\times(\Omega_{\rm HI}h/2.45\times10^{-4})(1+z)^2/E(z)$ | `hi_model.py:T_bar_b(z)` | 44 $\mu$K via `pinetti2022.T_bar_b_thesis()` | **Partial** | Equivalent: $188 \times 2.45\times10^{-4} \approx 0.046$ mK $= 46\,\mu$K $\approx 44\,\mu$K (rounding). Combined with computed vs fixed $\Omega_{\rm HI}$, numerical results differ slightly |

### 3.8 Effective HI bias

| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $b_{\rm HI} = (1/\bar\rho_{\rm HI})\int (dn/dM)\,M_{\rm HI}\,b(M)\,dM$ | Thesis Eq. 4.7 | `hi_model.py:b_HI(z)` | $q=0.75$ via `pinetti2022.b_HI_pinetti()` | **Match** | Via `scipy.quad` in log-mass |

### 3.9 HI power spectra

| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $P_{\rm HI}^{\rm 1h} = (1/\bar\rho_{\rm HI}^2)\int (dn/dM)\,M_{\rm HI}^2\,\tilde{u}_{\rm HI}^2\,dM$ | Thesis Eq. 4.13 | `hi_model.py:P_HI_1h` | Same | **Match** | Rectangle rule over log-mass grid (n_M=160) |
| $P_{\rm HI}^{\rm 2h} = [(1/\bar\rho_{\rm HI})\int (dn/dM)\,b\,M_{\rm HI}\,\tilde{u}_{\rm HI}\,dM]^2 P_{\rm lin}$ | Thesis Eq. 4.14 | `hi_model.py:P_HI_2h` | Same | **Match** | Same method |
...
| $k = (\ell+1/2)/\chi$ | LoVerde & Afshordi (2008); thesis uses $k = \ell/\chi$ | `angular_power.py:163` | $k = \ell/\chi$ | **Differs** | Pipeline improvement: more accurate at low $\ell$. ~5% at $\ell=10$, rapidly decreasing |
...
| Redshift integration: rectangle rule, $n_z=200$ uniform points | N/A (computational choice) | `angular_power.py:142-143` | Same | Appropriate | Sub-percent accuracy for typical band widths |
