...
| $\int 4\pi r^2 \rho^2 dr = \frac{4\pi}{3}\rho_s^2 r_s^3[1 - 1/(1+c)^3]$ | Analytic NFW result | `rho2_integral_analytic` line 31 | Same | **Match** | Verified: $\int_0^c (1+x)^{-4} dx = \frac{1}{3}[1-(1+c)^{-3}]$ |
| $\tilde{v}(k \mid M) = (4\pi/\bar\rho^2)\int r^2 \rho^2 \sin(kr)/(kr) dr$ | Thesis Eq. 5.1 implicit | `v_tilde` line 50 | Same | **Match** | Normalization: $\tilde{v}(k\to 0) = \int\rho^2 d^3x / \bar\rho^2$ |
| $\tilde{v}$ numerical: $r_{\rm min} = 10^{-6} r_s$ | Implementation | line 81 | Same | **Match** | Negligible error: $\rho^2 r^2 \to 0$ as $r\to 0$ |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $\log_{10} B = \sum_{i=0}^5 b_i [\log_{10}(M/M_\odot)]^i$ | Moline Eq. 18 | lines 118–120 | Same | **Match** | |
...
| $B(M,z) = B(M,z{=}0)/(1+z)$ | Moline thesis Eq. 3.48 | line 125 | Same | **Match** | |
| Polynomial argument in $M_\odot$ (not $M_\odot/h$) | Moline: masses in physical $M_\odot$ | `M_solar = M / cfg.h` line 106 | Same | **Match** | Resolved: now uses $M_{\rm phys} = M_{\rm code}/h$ consistently with `v_circ` |
| Valid range: $10^{-6} \lt M\,[M_\odot] \lt 10^{15}$ | Moline Sec. 3.2 | `np.clip(M_solar, 1e-6, 1e15)` line 117 | Same | **Match** | |
| Output zeroed for $M < M_{\rm min,sub}$ | Not in literature | `np.where(mask, B, 0.0)` line 127 | Same | **Differs** | Numerical safety; conservative scenario zeroes $M < 10^7\,M_\odot$ |
| Conservative: $B=0$ for $M \lt 10^7\,M_\odot$ | Pipeline design | lines 109–110 | Same | **Match** | |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $\Delta^2 = (1/\bar\rho_m^2)\int (dn/dM)(1+B)\int\rho^2 d^3x\, dM$ | Pinetti Eq. 4.2 | `_clumping_compute` line 135 (cached via `@_cache_stable`) | Same | **Match** | |
| Integration via $d\ln M$: factor of $M$ in integrand | Standard | line 149: `dn * (1+B) * rho2 * M` | Same | **Match** | |
| Integration limits from config | `M_MIN_DM=1e-6`, `M_MAX_DM=1e18` | Uses config values directly at lines 157–195 | Same | **Match** | Resolved: silent clamping removed |
...
| Normalization by $\bar\rho_m^2$ and $(1+z)^3$ | Pinetti Eq. 4.2 | `cfg.RHO_BAR**2` and `(1+z)**3` line 154 | Same | **Match** | |
...
| Table columns: $[m_{\rm DM},\, \log_{10}x,\, {\rm channels}]$ | PPPC4DMID format | Parser at lines 43--90 | Same | **Match** | |
| $dN/d(\log_{10}x)$ is the tabulated quantity | PPPC4DMID convention | Interpolators on line 86 | Same | **Match** | |
| $dN/dx = dN/d(\log_{10}x) / (x\,\ln 10)$ | Standard calculus | line 212 | Same | **Match** | |
| $dN/dE = (dN/dx) / m_\chi$ | $x = E/m_\chi$ | line 245 | Same | **Match** | |
| Interpolation in $\log_{10}$ space | Implementation | `RectBivariateSpline` on $\log_{10}$ grid (lines 86--88) | Same | **Match** | Cubic $k=3$ |
| Mass range clamped to table | Implementation | `np.clip(log10m, ...)` line 137 | Same | **Match** | |
| Energy: $E' = (1+z)E_\gamma$ | Pinetti Eq. 4.1 | `E_emit = E_GeV * (1.0 + z)` dm_model.py:199 | Same | **Match** | |
| Early return if $dN/dE \le 0$ | Optimization | dm_model.py:201--202 | Same | **Match** | |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| Model: Dominguez et al. (2011) | Pipeline default | `model='dominguez'` line 30 | Same | **Match** | |
| $\tau(E_\gamma, z)$ at **observed** energy | Dominguez / `ebltable` convention | dm_model.py:204 passes `E_GeV` | Same | **Match** | |
| `ebltable` API: energy in TeV | Package docs | `E_TeV = E_GeV / 1000.0` line 54 | Same | **Match** | |
| $e^{-\tau}$ attenuation factor | Standard | `np.exp(-tau(...))` line 61 | Same | **Match** | |
| $\tau \ge 0$ enforced | Physical | `tau()` returns raw `ebltable` values (non-negative by construction) line 56 | Same | **Match** | |
| $\tau(z \le 0) = 0$ | No local absorption | lines 50--51 | Same | **Match** | |
| Analytic fallback | Pipeline convenience | Removed (no longer present) | Same | **Differs** | Not from Dominguez; undocumented calibration |
| Alternative models available | Feature | `model` parameter (lines 39--41) | Same | **Match** | Finke, Franceschini, Saldana-Lopez 2021 |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $1/(8\pi)$ prefactor | Pinetti Eq. 4.1: $1/(4\pi) \times 1/(2m_\chi^2)$ | `sigma_v / (8 * np.pi)` line 210 | Same (no override yet) | **Match** | $1/(4\pi) \times 1/2 = 1/(8\pi)$; Majorana DM |
| $(\rho_{\rm DM}/m_\chi)^2$ | Pinetti Eq. 4.1 | line 211 | Same | **Match** | |
...
| $(1+z)^3$ cosmological factor | Pinetti Eq. 4.1 | line 212 | Same | **Match** | |
...
| Final unit conversion to $({\rm Mpc}/h)^{-3}$ | Per-$\chi$ convention | `W_cgs * Mpc_h_cm**3` line 216 | Same | **Match** | |
| $dN/dE$ at rest-frame energy | Pinetti Eq. 4.1 | `E_emit = E_GeV * (1+z)` line 199 | Same | **Match** | |
| EBL at observed energy | Standard convention | line 204 | Same | **Match** | |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|------------------------|--------|-------|
| $P^{2h} = I_{\rm DM} \times I_{\rm HI} \times P_{\rm lin}$ | Pinetti Eqs. 5.1--5.2 | line 51 | Same (no override yet) | **Match** | |
| $I_{\rm DM} = \int (dn/dM)\,b\,\tilde{v}/\Delta^2\,dM$ | Eq. 5.1 | lines 36–43 | Would use $q=0.75$ bias | **Match** | $\tilde{v}/\Delta^2$ is the correct normalization |
...
| Limber $k = (\ell+1/2)/\chi$ | LoVerde & Afshordi (2008) | line 175 | $k = \ell/\chi$ via `pinetti2022.limber_k()` | **Differs** | Pipeline improvement; ~5% at $\ell=10$, negligible at $\ell \gt 100$ |
| $d\chi/dz = c \cdot h / H(z)$ in [Mpc/h] | Standard | line 169 | Same | **Match** | |
...
| EBL energy convention: observed or emitted? | Observed (confirmed by `ebltable` API and code comments at dm_model.py:204) | Same |
