...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|-------------------------|--------|-------|
| Step 1: Di Mauro Eq. C.13: $\log L_\gamma = 2.0 + 1.008\log(\nu L_\nu)_{\rm core}$ [erg/s] | Di Mauro+ (2014) Eq. C.13 | `_L151_from_Lgamma` | Same | **Match** | |
...
| Step 2: Lara Eq.: $\log L_{\rm core}^{5{\rm GHz}} = 4.2 + 0.77\log L_{\rm tot}^{1.4{\rm GHz}}$ [W/Hz] | Lara+ (2004) | `_L151_from_Lgamma` | Same | **Match** | |
| $L_{\rm core}$ intercept: 4.2 | Lara+ (2004) | `LARA_A = 4.2` | Same | **Match** | |
| $L_{\rm core}$ slope: 0.77 | Lara+ (2004) | `LARA_B = 0.77` | Same | **Match** | |
| Step 3: Freq scaling $L_{1.4} = L_{151}(1400/151)^{-\alpha_r}$ | Inoue (2011) | `_L151_from_Lgamma` | Same | **Match** | |
| Radio spectral index $\alpha_r = 0.80$ | Inoue (2011) | `RADIO_ALPHA = 0.80` | Same | **Match** | |
| Composite log-space Jacobian: $1/(0.77 \times 1.008)$ | Analytic from chain | `_L151_from_Lgamma` with `1.0 / (DIMAURO_B * LARA_B)` | Same | **Match** | |
| $dL_{151}/dL_\gamma = (L_{151}/L_\gamma) \times {\rm dlog\ ratio}$ | Standard calculus | `_L151_from_Lgamma` | Same | **Match** | |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|-------------------------|--------|-------|
| Full assembly formula $\phi_\gamma = k\eta(1+z)^{-(2-\Gamma)}\rho_r/(\ln 10\,L_{151})\,\lvert dL_{151}/dL_\gamma \rvert$ | Di Mauro+ (2014) Eq. C.19 | `_glf_mAGN` | Same | **Match** | |
...
| K-correction $(1+z)^{-(2-\Gamma)} = (1+z)^{0.37}$ | Di Mauro+ (2014) Eq. C.19 | `_glf_mAGN` uses `(1+z)**(2-Gamma)` | Same | **Match** | |
...
| Energy normalization $I_\alpha = \int_{0.1}^{100} E^{1-\alpha}dE$ [GeV² band] | Pinetti+ (2020) | `W_gamma_astro` analytic `energy_integral` | Same | **Match** | Analytic form for $\alpha\ne 2$ |
