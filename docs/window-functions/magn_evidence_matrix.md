...
|-------|---------------------|----------|-------------------------|--------|-------|
| Two-component form: $\rho_r = \rho_l + \rho_h$ | Willott+ (2001) Table 1; Di Mauro+ (2014) Eq. C.9 | `_willott_rlf` lines 209–227 sums low + high components | Same | **Match** | |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|-------------------------|--------|-------|
| Step 1: Di Mauro Eq. C.13: $\log L_\gamma = 2.0 + 1.008\log(\nu L_\nu)_{\rm core}$ [erg/s] | Di Mauro+ (2014) Eq. C.13 | `_L151_from_Lgamma` lines 287–288 | Same | **Match** | |
...
| Step 2: Lara Eq.: $\log L_{\rm core}^{5{\rm GHz}} = 4.2 + 0.77\log L_{\rm tot}^{1.4{\rm GHz}}$ [W/Hz] | Lara+ (2004) | `_L151_from_Lgamma` lines 294–296 | Same | **Match** | |
| $L_{\rm core}$ intercept: 4.2 | Lara+ (2004) | `LARA_A = 4.2` | Same | **Match** | |
| $L_{\rm core}$ slope: 0.77 | Lara+ (2004) | `LARA_B = 0.77` | Same | **Match** | |
| Step 3: Freq scaling $L_{1.4} = L_{151}(1400/151)^{-\alpha_r}$ | Inoue (2011) | `_L151_from_Lgamma` lines 298–300 | Same | **Match** | |
| Radio spectral index $\alpha_r = 0.80$ | Inoue (2011) | `RADIO_ALPHA = 0.80` | Same | **Match** | |
| Composite log-space Jacobian: $1/(0.77 \times 1.008)$ | Analytic from chain | `_L151_from_Lgamma` lines 305–306 with `1.0 / (DIMAURO_GAMMA_RADIO_B * LARA_B)` | Same | **Match** | |
| $dL_{151}/dL_\gamma = (L_{151}/L_\gamma) \times {\rm dlog\ ratio}$ | Standard calculus | `_L151_from_Lgamma` line 306 | Same | **Match** | |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|-------------------------|--------|-------|
| Assembly formula $\phi_\gamma = k\eta(1+z)^{-(2-\Gamma)}\rho_r/(\ln 10\,L_{151})\,\lvert dL_{151}/dL_\gamma \rvert$ | [Pinetti (2022)](../literature/pinetti2022_thesis.md) Eq. C.19 | `_glf_mAGN` lines 311–343 | Same | **Match** | Includes K-correction factor |
...
| K-correction $(1+z)^{-(2-\Gamma)}$ | [Pinetti (2022)](../literature/pinetti2022_thesis.md) Eq. C.19 | `_glf_mAGN` line 337: `K_corr = (1+z)**(-(2-alpha))` | Same | **Match** | Accounts for observed-to-rest-frame luminosity mapping |
...
| Energy normalization $I_\alpha = \int_{0.1}^{100} E^{1-\alpha}dE$ [GeV² band] | Pinetti+ (2020) | `_W_gamma_astro_impl` analytic `energy_integral` lines 700–703 | Same | **Match** | Analytic form for $\alpha\ne 2$ |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|-------------------------|--------|-------|
| $M_\star = 10^9 M_\odot (L_\gamma/10^{48})^{0.36}$ | Di Mauro+ (2014) Eq. C.21 | `MAGN_MSTAR_*` in `bias_astro` line 794 | Same | **Match** | |
| $M_{\rm halo} = 10^{13} M_\odot (M_\star/[10^{8.8}(1+z)^{1.4}])^{0.645}$ | Di Mauro+ (2014) Eq. C.20 | `MAGN_MHALO_*` in `bias_astro` lines 795–798 | Same | **Match** | |
...
| T5 | Unit mismatch in Di Mauro config comment | Documentation | Low | `config.py` lines 195–196 comment says "log L_gamma [erg/s] = ... log L_core [W/Hz]" but actual Di Mauro correlation is in $\nu L_\nu$ [erg/s]. Code handles correctly |
| ~~T6~~ | ~~K-correction $(1+z)^{-(2-\Gamma)}$ missing from `_glf_mAGN`~~ | — | — | **Resolved:** K-correction now included in `_glf_mAGN` (line 337) per Pinetti (2022) Eq. C.19 |
...
| ~~5~~ | ~~K-correction $(1+z)^{-(2-\Gamma)}$ missing~~ | Di Mauro Eq. C.19 | Now present in `_glf_mAGN` line 337 | **Resolved** | — |
...
| K-correction sign | $(1+z)^{-(2-\Gamma)} = (1+z)^{-(2-2.37)} = (1+z)^{+0.37}$. This is a positive correction (boost at high $z$), as expected for $\Gamma \gt 2$. Now present in `_glf_mAGN` line 337 |
