...
|-------|---------------------|----------|-------------------------|--------|-------|
| Two-component form: $\rho_r = \rho_l + \rho_h$ | Willott+ (2001) Table 1; Di Mauro+ (2014) Eq. C.9 | `_willott_rlf` sums low + high components | Same | **Match** | |
...
| Willott reference cosmology | Willott (2001): $H_0=50$ km/s/Mpc | `H0_WILLOTT = 50.0` | Same | **Match** | |
| Willott $H_W(z)$ formula | — | $H_0\sqrt{(1+z)^3}$ (EdS, $\Omega_M=1$) | Same | **Investigate** | **The code uses EdS form $H(z)=H_0(1+z)^{3/2}$.** Literature doc `willott2001.md` claims Model C ($\Omega_M=0$, which would give $H=H_0(1+z)$), but the parameter values in `config.py` match Willott Table 1 Model A (EdS). The code and parameters are internally self-consistent (both EdS); the literature doc contains an error claiming Model C |
| LRU caching (maxsize=512) | — | `@functools.lru_cache(maxsize=512)` | Same | N/A | Pipeline-only optimization |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|-------------------------|--------|-------|
| Step 1: Di Mauro Eq. C.13: $\log L_\gamma = 2.0 + 1.008\log(\nu L_\nu)_{\rm core}$ [erg/s] | Di Mauro+ (2014) Eq. C.13 | Lines 193-196 | Same | **Match** | |
| Unit: Di Mauro correlation uses $\nu L_\nu$ [erg/s], not $L_\nu$ [W/Hz] | Di Mauro+ (2014) | Code correctly converts; config comment line 152 says "W/Hz" (misleading) | Same | **Partial** | **Config comment is misleading.** Code correctly handles the unit conversion (line 199: `L_WHZ = nuLnu/(nu*1e7)`), but `config.py:152` comment "log L_core [W/Hz]" is wrong — the Di Mauro correlation is between $L_\gamma$ [erg/s] and $\nu L_\nu$ [erg/s]. Config comment should be corrected |
...
| Step 2: Lara Eq.: $\log L_{\rm core}^{5{\rm GHz}} = 4.2 + 0.77\log L_{\rm tot}^{1.4{\rm GHz}}$ [W/Hz] | Lara+ (2004) | Lines 202-204 | Same | **Match** | |
| $L_{\rm core}$ intercept: 4.2 | Lara+ (2004) | `LARA_A = 4.2` | Same | **Match** | |
| $L_{\rm core}$ slope: 0.77 | Lara+ (2004) | `LARA_B = 0.77` | Same | **Match** | |
| Step 3: Freq scaling $L_{1.4} = L_{151}(1400/151)^{-\alpha_r}$ | Inoue (2011) | Line 207: `(1400/151)**RADIO_ALPHA` | Same | **Match** | |
| Radio spectral index $\alpha_r = 0.80$ | Inoue (2011) | `RADIO_ALPHA = 0.80` | Same | **Match** | |
| Composite log-space Jacobian: $1/(0.77 \times 1.008)$ | Analytic from chain | Line 213: `1.0/(DIMAURO_B * LARA_B)` | Same | **Match** | |
| $dL_{151}/dL_\gamma = (L_{151}/L_\gamma) \times {\rm dlog\ ratio}$ | Standard calculus | Line 214 | Same | **Match** | |
...
| $d\Phi/dL$ from $d\Phi/d\log L$: divide by $L\ln 10$ | Standard | Line 241 | Same | **Match** | |
...
| $L_{\rm up} = \min(L_{\max}, L_{\rm thr})$ | Pinetti+ (2020) | Line 525 | Same | **Match** | |
...
| Claim | Literature Reference | Pipeline | Pipeline (Pinetti 2022) | Status | Notes |
|-------|---------------------|----------|-------------------------|--------|-------|
| $M_\star = 10^9 M_\odot (L_\gamma/10^{48})^{0.36}$ | Di Mauro+ (2014) Eq. C.21 | `MAGN_MSTAR_*` in `bias_astro` | Same | **Match** | |
| $M_{\rm halo} = 10^{13} M_\odot (M_\star/[10^{8.8}(1+z)^{1.4}])^{0.645}$ | Di Mauro+ (2014) Eq. C.20 | `MAGN_MHALO_*` in `bias_astro` | Same | **Match** | |
...
| T5 | Willott $\Omega_M=0$ vs $\Omega_M=1$ ambiguity | Documentation | Moderate | Literature doc `willott2001.md` claims Model C ($\Omega_M=0$). Code/config use EdS ($\Omega_M=1$) for both parameters and volume correction. Parameter values verified to match Model A (EdS). **Literature doc has an error** |
| T6 | Unit mismatch in Di Mauro config comment | Documentation | Low | `config.py:152` says "W/Hz" but actual correlation is in erg/s ($\nu L_\nu$). Code handles correctly |
...
| # | Item | Method | Impact |
|---|------|--------|--------|
| C1 | Volume correction $\eta(z)$ cached | `lru_cache(maxsize=512)` | Performance only |
| C2 | Cosmology Jacobian via `scipy.quad` | `limit=100` in `_willott_volume_correction` | Adequate for smooth integrand |
...
| C5 | $d_L$ computed in physical Mpc (divide by $h$) | Line 517 | Explicit h-factor handling |

---

## 9. Pipeline (Pinetti 2022) Parallel Summary

The Pinetti 2022 parallel implementation makes **no mAGN-specific deviations** from the pipeline. The complete chain (Willott RLF → Inoue freq scaling → Lara core-total → Di Mauro gamma-radio → Pinetti window function) matches the thesis exactly. The only differences inherited from the halo model are:
...
|---|------|-------------------|----------|--------|----------|
| 1 | Willott cosmology model | Doc says Model C ($\Omega_M=0$); thesis uses (likely) Model A (EdS) | EdS (Model A) in code + parameters | **Doc error** | Moderate (documentation) |
| 2 | Di Mauro correlation units | Di Mauro Eq. C.13 uses $\nu L_\nu$ [erg/s] | Code correct, config comment wrong | Documentation | Low |
| 3 | Fixed $L^{\rm char}$ for bias | Not prescribed | $10^{44}$ erg/s | Simplification | Minor |
| 4 | Pure power-law spectrum | Standard choice | Same | Simplification | Minor |
| 5 | Extrapolation to high $z$ | Willott calibrated to $z\lesssim 4$ | Extrapolates silently | Simplification | Low |
...
5. Verify $\eta(z=1)$ numerically (expected ~0.1-0.3 for EdS→Planck)
