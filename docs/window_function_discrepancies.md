...
Systematic comparison of our pipeline's normalized window functions against Pinetti (2022, arXiv:2212.00125) Figure 5.1.
...
- **BL Lac:** Replaced Di Mauro+(2013) HSP+LISP reparameterization with single-component Ajello+(2014) piecewise LDDE
...
|---|-------|-----------|----------|-------|
| H1 | W_HI = T̄_b · b_HI · φ · H/(ch) | Pinetti Eq. 3.15 | Same | YES |
| H2 | T̄_b = 188 h Ω_HI (1+z)²/E(z) mK | Pinetti Eq. 3.4 | ~180 h mK (4% low) | YES |
...
| H5 | v_{c,0} range 36–102 km/s | Padmanabhan+ (2017) | 101.61 (upper extreme) | PARTIAL |
...
| # | Claim | Reference | Our Code | Match |
|---|-------|-----------|----------|-------|
| B1 | W = d_L²/(1+z)² × ∫Φ dF/dE dL | Pinetti Eq. 4.3 | Same (after algebraic simplification) | YES |
| B2 | LDDE from Ajello+ (2014) | 211 BL Lacs, 1LAC catalog | HSP+LISP from Di Mauro+(2013) | PARTIAL |
| B3 | HSP negative evolution p₁=−1.64 | Ajello+(2014) | Same | YES |
| B4 | Window peaks at z~1 | Figure 5.1 | Peaks at z~0.1 | **MISMATCH** |

**Root cause:** The Di Mauro+(2013) reparameterization of the Ajello+(2014) BL Lac GLF uses a "sum" evolution form with HSP p₁=−1.64 (negative), which makes HSP BL Lac density increase toward z=0. This overwhelms the LISP component, concentrating all BL Lac emission at very low z. The original Ajello+(2014) paper may use a different parameterization (e.g., PLE or LDDE with different form) that produces a peak at z~1.
...
| # | Claim | Reference | Our Code | Match |
|---|-------|-----------|----------|-------|
| F1 | LDDE from Ajello+ (2012) Table 3 | Direct paper values | Same | YES |
...
| # | Claim | Reference | Our Code | Match |
|---|-------|-----------|----------|-------|
| D1 | W = (σv/8π)(ρ/m)²(1+z)³/H × Δ² × dN/dE × e^{-τ} | Pinetti Eq. 4.1 | Same structure | YES |
...
| # | Claim | Reference | Our Code | Match |
|---|-------|-----------|----------|-------|
| M1 | mAGN from Di Mauro+(2014) radio LF | L_γ–L_radio correlation | Hand-calibrated LDDE | **MISMATCH** |
| S1 | SFG from Gruppioni+(2013) IR LF × L_γ∝L_IR^1.17 | Herschel PEP/HerMES | Hand-calibrated LDDE | **MISMATCH** |
...
| RC2 | BL Lac HSP GLF parameters from indirect source | BL Lac | HIGH | Need original Ajello+(2014) fit |
