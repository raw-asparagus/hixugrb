...
Table 8 also gives the redshift evolution parameters for each class. **Resolved (2026-04-20 PDF audit):** The source PDF Table 8 shows that **only the spiral row** has entries for $k_{L,2}$ (= 0.00 ± 0.46) and $z_{b,L}$ (= 1.1). The starburst, SF-AGN, AGN2, and AGN1 rows have **blank entries** for $k_{L,2}$ and $z_{b,L}$, meaning they use a single power-law for luminosity evolution (no break). The pipeline's current implementation — applying the $z = 1.1$ luminosity break only to spiral — is correct per the paper. D13 is confirmed resolved.
